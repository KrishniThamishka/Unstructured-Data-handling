...
parser.add_argument('--format', choices=['csv', 'json'], default='csv', help='d2_SRT_List2.py')
parser.add_argument('filename', help='d2_SRT_List2.py')
...
df = pd.DataFrame(columns=['Frame No', 'Time code', 'latitude', 'Longitude', 'Altitude', 'Speed', 'Distance'])
...
#print(df)
if args.format == 'csv':
    df.to_csv('resources/subtitle/output.csv', index=False)
    print('Data written to output.csv')
elif args.format == 'json':
    df.to_json('resources/subtitle/output.json', orient='records')
